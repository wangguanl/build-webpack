...
function generateUtils() {
  ...
  const dependOn = [],
    entryUtils = {},
    modules = ['lodash'],
    utils = ['deep-copy', 'parse-url', 'verify-var', 'unique', 'validation', 'cookies']

  modules.forEach(moduleName => entryUtils[moduleName] = moduleName)
  utils.forEach(fileName => {
    const filePath = utilsResolvePath(fileName);
    dependOn[`shared-util_${fileName}`] = filePath;
    entryUtils[fileName] = filePath;
  });
...
module.exports = {
  ...
  plugins: [
    ...
    /* new HtmlWebpackPlugin({
      filename: "test.html",
      template: viewsResolvePath("/test/index.html"), // 自定义 html 模板
      publicPath: "",
